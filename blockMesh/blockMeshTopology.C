...
\\      /  F ield         | Unsupported Contributions for OpenFOAM
 \\    /   O peration     |
  \\  /    A nd           | Copyright (C) 2011 OpenFOAM Foundation
   \\/     M anipulation  |
-------------------------------------------------------------------------------
2013-06-01 Karl-Johan Nogenmyr. Added implmentation of readSnapFaces(...)
...
void Foam::blockMesh::readSnapFaces
(
    const IOdictionary& meshDescription 
)
{
    const dictionary& blockDict(meshDescription);
    const dictionary& topDict(blockDict.subDict("snapFaces"));
    label surfI = 0;
    label nSurfaces = topDict.size();
    snapSurfaces_.clear();
    snapSurfaces_.setSize(nSurfaces);
    faceListList snapFaces(nSurfaces);
    
    Info << "Reading in " << nSurfaces << " surfaces" << endl;
    ...
    forAllConstIter(dictionary, topDict, iter)
    {
        ...
        dict.lookup("faces") >> snapFaces[surfI];

        surfI++;
    }
    
    forAll(topologyPtr_->faces(), faceI)
    {
        forAll(snapFaces, surfI)
        {
            forAll(snapFaces[surfI], snapfaceI)
            {
                bool match = true;
                forAll(snapFaces[surfI][snapfaceI], point1)
                {
                    label face1p = snapFaces[surfI][snapfaceI][point1];
                    bool pointFound = false;
                    forAll(topologyPtr_->faces()[faceI], point2)
                    {
                        label face2p = topologyPtr_->faces()[faceI][point2];
                        if(face1p == face2p)
                        {
                            pointFound = true;
                        }
                ...
                if (match) // faceI is a face that should snap to surfI
                {
                    projectionList_[faceI] = surfI;
                    vector v1 = 
                    (
                        topologyPtr_->points()[snapFaces[surfI][snapfaceI][1]]
                      - topologyPtr_->points()[snapFaces[surfI][snapfaceI][0]]
                    );
                    vector v2 = 
                    (
                        topologyPtr_->points()[snapFaces[surfI][snapfaceI][2]]
                      - topologyPtr_->points()[snapFaces[surfI][snapfaceI][1]]
                    );
                    faceNormal_[faceI] = v1 ^ v2;
                    break;
                }
                else
                {
                    projectionList_[faceI] = -1;
                }
            }
            if(projectionList_[faceI] >= 0) break;
        }
    }
}
